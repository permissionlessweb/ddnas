...
- To prevent DDNAS admins from scraping all registered API keys from the DB deployed, we can create a workflow that will mitigate this risk.
First, we must have a secret network contract that lets anyone register a dnas admin address as an authorized entity for query permits to retrive API key values. This dnas admin key is able to retrieve the API keys from the state, only if replay attack proof data is included that is generated by the eligible entity making use of a registered dnas keys. The DNAS API will hold internally the dnas admin keys, and facilitate the query permits to secret networ
